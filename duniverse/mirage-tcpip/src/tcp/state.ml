...
let t ~clock ~on_close =
  { clock; on_close; state=Closed }
...
let rec finwait2timer t count timeout =
  ...
  Eio.Time.sleep t.clock (Int64.to_float timeout /. 1_000_000_000.);
  match t.state with
  | Fin_wait_2 i ->
...
let timewait t twomsl =
  ...
  Eio.Time.sleep t.clock (Int64.to_float twomsl /. 1_000_000_000.);
  t.state <- Closed;
  Log.debug (fun fmt -> fmt "timewait on_close");
  t.on_close ();
  ()

let tick ~sw t (i:action) =
  ...
  let tstr s (i:action) =
    ...
    | Fin_wait_1 a, Recv_ack b ->
      if diffone b a then
        let count = 0 in
        Eio.Fiber.fork ~sw (fun () -> finwait2timer t count fin_wait_2_time);
        Fin_wait_2 count
      else
        Fin_wait_1 a
    ...
    | Fin_wait_2 _, Recv_fin ->
      Eio.Fiber.fork ~sw (fun () -> timewait t time_wait_time);
      Time_wait
